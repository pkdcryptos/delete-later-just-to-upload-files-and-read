class Traade < ActiveRecord::Base
  ...
  scope :m1, -> { where("created_at > ?", 1.minutes.ago) }
  
  #after_commit  :dbg_record
  ...
  def for_global
    {
      ...
      isBuyerMaker:   trend == 'up' ? false : true,
      date:   created_at.to_i,
      price:  price.to_s || ZERO,
      qty: volume.to_s || ZERO,
      time: tradetime,      
    }
  end
  ...
  def dbg_record
     #Rails.logger.info "DBG: Traade record: id: #{self.id}, price: #{price}, volume: #{volume}, ask_id: #{ask_id}, bid_id: #{bid_id}, ask_member_id: #{ask_member_id}, bid_member_id: #{bid_member_id}, funds: #{funds}"
  end
