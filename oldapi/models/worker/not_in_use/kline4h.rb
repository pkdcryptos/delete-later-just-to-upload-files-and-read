module Worker
  ...
  def redis
    ...
    def process(payload, metadata, delivery_info)
      ...
      create_global_keys_kline4h 
    end
    

    
    def create_global_keys_kline4h
        lasttid = @trade.id  
		
		#Rails.logger.info "DBG: KLINE-WRITER: kline4h received trade : #{lasttid}"	
    	...
    	#4h pusher
    	t4h = (trade_time/(240*60)).to_i*240*60
    	key4h = "ktio:#{@trade.market}:4h:#{t4h}"
		tdiff = t1h-t4h
    	limit = (tdiff/3600)-1    	
    	...
    	_, _, high, low, _, volumes = tsarray.transpose
  		output4h = [t4h, tsarray.first[1], high.max, low.min, tsarray.last[4], volumes.sum.round(8), lasttid]	
    	Rails.cache.write key4h, output4h, expires_in: 60.minutes
    	
    	
    	
    	#Rails.logger.info "DBG: KLINE-WRITER: kline4h completed trade : #{lasttid}"
