module APIv1
  module Helpers
    ...
    def current_user
			Rails.logger.info "DBG: current_token: #{current_token}"
      @current_user ||= current_token.try(:member)
    end

    def current_token
			Rails.logger.info "DBG: api_v1.token: #{env['api_v1.token']}"
      @current_token ||= env['api_v1.token']
    end
    ...
    def get_k_json
      key = "ktio:#{params[:market]}:k:#{params[:period]}"
